...
| **BG-INT-005** | Visibility Preservation Layer | Visibility Preservation Layer (VPL) | Visibility Preservation Layer (VPL) | `PRESERVED` | Architecture section | None | Removed parenthetical translation `Sichtbarkeitsebene` | Verified |
| **BG-INT-006** | Experience Preservation Layer | Experience Preservation Layer (EPL) | Experience Preservation Layer (EPL) | `PRESERVED` | Architecture section | None | Removed parenthetical translation `Verhaltensebene` | Verified |
| **BG-INT-007** | Design Preservation Layer | Design Preservation Layer (DPL) | Design Preservation Layer (DPL) | `PRESERVED` | Architecture section | None | Removed parenthetical translation `Gestaltungsebene` | Verified |
...
| **BG-INT-018** | Repository paths | `/src/workspace/`, `/src/workflow/`, `/backend/app/policies/`, `/tooling/governance/`, `/tooling/analyzer/` | Removed from public page prose; retained in private evidence. | `REPOSITIONED PRIVATELY` | Implementation prose / evidence map | Path exposure drift | Replaced raw paths with generic descriptions on public page | Verified |
...
| **BG-INT-020** | Results and metrics | 45% time savings, 100% merges, 0 leaks, SQ Gate A | 45% Zeitersparnis, 100% Code-Übergaben im Pilotlauf, keine Übertragungen außerhalb der Gateway-Regeln festgestellt, Quality Gate A | `PRESERVED WITH LANGUAGE QUALIFICATION` | Results section (scoped to pilot) | None | Qualified security metrics to pilot-observed data flows | Verified |
| **BG-INT-021** | Risk model | Three core risk definitions and mitigations | 3 Risks (Knowledge Cutoff, Code Bloat, Test Blind Spots) in risk table | `PRESERVED` | Risks section | Format change | Converted risks bullet list to public table; mapped to internal risk IDs | Verified |
